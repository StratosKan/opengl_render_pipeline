...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Second OpenGL Window with Geometry", NULL, NULL);
	if (window == NULL)
	{
	...
	//delete shaders. We have a linked program now
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindVertexArray(VAO); // We only have a single VAO, so there is no need to bind it every time. However, we will need to do it in more complicated scenarios
	...
	// De-allocate all buffers once we have no use for them. In most cases, the OS will de-allocate them itself
